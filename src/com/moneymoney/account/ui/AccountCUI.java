...
public class AccountCUI {
	...
	private static void updateAccount() {
		...
		case 1:
			...
			try {
				savingsAccount = savingsAccountService.updateAccount(accountNumber ,input, newName );
		...
		case 2:
			...
			String salary = scanner.next().equalsIgnoreCase("n")?"false":"true";
 			try {
				savingsAccount = savingsAccountService.updateAccount(accountNumber ,input, salary );
				System.out.println(savingsAccount);
			} catch (ClassNotFoundException | SQLException
					| AccountNotFoundException e) {
				e.printStackTrace();
	...
	private static void createSavingsAccount(String accountHolderName, double accountBalance, boolean salary) {
		try {
			SavingsAccount newAccount =  savingsAccountService.createNewAccount(accountHolderName, accountBalance, salary);
